...
export const MobileStickyActionBar: React.FC<MobileStickyActionBarProps> = ({
  ...
  return (
    ...
    >
      <div className="max-w-md mx-auto pointer-events-auto bg-white border border-taj-gray-border rounded-2xl p-3 flex items-center justify-between gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            {breakfastIncluded && (
              <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[8px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 flex-shrink-0">
                Breakfast
              </span>
            )}
          </div>
          <div className="flex items-baseline gap-1 mt-0.5">
            <span className="text-base font-bold font-serif text-taj-burgundy tabular-nums leading-tight">
              {currency}{typeof pricePerNight === 'number' && !isNaN(pricePerNight) ? pricePerNight.toLocaleString('en-IN') : '—'}
            </span>
        ...
        <a
          ...
          className="min-h-[48px] px-5 bg-taj-burgundy hover:bg-taj-burgundy-deep text-white font-medium text-xs rounded-xl flex items-center justify-center gap-1.5 active:scale-95 transition-all duration-150 flex-shrink-0"
        >
